...
def find_latest_log(exp_name, log_dir):
    """Find the log file with the most training steps (handles multiple job submissions)"""
    best_file = None
    max_rewards = 0
            ...
            try:
                with open(f) as file:
                    ...
                    if "Train step" in content:
                        reward_count = len(re.findall(r'(?:ppo_actor/)?task_reward/avg', content))
                        if reward_count > max_rewards:
                            max_rewards = reward_count
                            best_file = f
            except:
                pass
...
def parse_log(log_file):
    with open(log_file) as f:
        content = f.read()
    rewards = [float(x) for x in re.findall(r'(?:ppo_actor/)?task_reward/avg\s*│\s*([\d.e+\-]+)', content)]
    ...
    # Find last non-zero reward for crashed jobs
    final_reward = None
    if rewards:
        ...
        if final_reward is None:
            final_reward = rewards[-1]  # Use last reward if all are zero
    ...
    train_positions = [m.start() for m in re.finditer(r'(?:ppo_actor/)?task_reward/avg', content)]
...
# ============================================================
# Plot 1: Reward curves per algorithm (4 subplots)
# ============================================================
...
# ============================================================
# Plot 3: Final reward heatmap (4x3 grid)
# ============================================================
...
for i, algo in enumerate(['grpo', 'cispo', 'sapo', 'm2po']):
    for j, eta in enumerate([0, 2, 4]):
        exp = f"{algo}_eta{eta}"
        if exp in data and data[exp]['rewards']:
            matrix[i, j] = data[exp].get('final_reward', data[exp]['rewards'][-1])
...
for algo in ['grpo', 'cispo', 'sapo', 'm2po']:
    ...
    for eta in [0, 2, 4]:
        exp = f"{algo}_eta{eta}"
        if exp in data and data[exp]['rewards']:
            rewards.append(data[exp].get('final_reward', data[exp]['rewards'][-1]))
